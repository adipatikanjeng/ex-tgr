...
<div class="account_form after_clear" style="margin:20px 0 0 0">
    <form action="{{ url(App::getLocale().'/my-account/tracking-order') }}" method="POST">
        {!! csrf_field() !!}
        <div class="row_input">
            <div class="col_input">
        ...
        <div class="row_input">
            <div class="col_input" style="margin-top:15px;">
                ...
                <!-- <a href="" class="readmore type_2" style="margin-left:10px;">Show Chart</a> -->
            </div>
        </div>
    </form>
...
<div class="box_vm box_result after_clear" style="">
    <table class="tb_biodata" style="width:100%;">
        <tr>
            <th width="60%">Nomor KP</th> <td width="5%">:</td> <td>{{ $row->skh_so_kp_number }}</td>
        </tr>
        <tr>
		...
		@if($row->skh_resi != '')
		<tr>
            <th width="60%">Nomor Resi</th> <td width="5%">:</td> <td>{{ $row->skh_resi }}</td>
        </tr>
		@endif
...
<div class="boxWizardStat after_clear">
    ...
    <div class="{{ ($row->skh_status_of_delivery == 'D') ? 'btn_wizard_active' : 'btn_wizard_inactive'}}"><a>Delivered</a>{{ Carbon::parse($row->skh_delivery_date)->format('d M Y') }}</div>
    <div class="{{ ($row->ssh_status_of_pod == 'O') ? 'btn_wizard_active' : 'btn_wizard_inactive'}}"><a>Received</a>{{ Carbon::parse($row->ssh_pod_date)->format('d M Y') }}</div>
</div>
@endif
@if(@$status == 'entry')
<div style="border-bottom:1px solid #ddd;margin-top:60px;"></div>
<div class="box_vm box_result after_clear" style="">
    <table class="tb_biodata" style="width:100%;">
        <tr>
            <th width="60%">KP Number</th> <td width="5%">:</td> <td>{{ @$row->contract_number }}</td>
        </tr>
        <tr>
            <th width="60%">Customer</th> <td>:</td> <td>{{ @$row->name }}</td>
        </tr>
        <tr>
