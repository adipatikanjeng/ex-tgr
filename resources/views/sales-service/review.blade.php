...
<br> <br>
@if(@$row->id_card)
<?php $file = explode('_', $row->id_card) ?>
<?php $file = (count($file) <= 2) ?  '_'.end($file) : $row->id_card?>
<br><br>
...
<?php $file = explode('_', $row->file_transfer) ?>
<?php $file = (count($file) <= 2) ?  '_'.end($file) : $row->file_transfer?>
<br><br>
Download File: <br><a href="{{ url(App::getLocale().'/download-file/'.$file) }}">{{ @$file }}</a>
@endif
